...
if (count _positionTel > 0) then
	...
	if (_positionTel distance getMarkerPos _base < 50) then
		...
		if (!_esHC) then {disableUserInput true; cutText [format ["Fast traveling, travel time: %1s , please wait", _distanceX],"BLACK",1];} else {hcShowBar false;hcShowBar true;["Fast Travel", format ["Moving group %1 to destination",groupID _groupX]] call A3A_fnc_customHint;};
 		if (!_esHC) then
 			{
 			_timePassed = 0;
			_distanceX = 0;
